...
# --- Samplers ---
def tick_sampler_with_energy(J, h, beta_fn, tau_ns, sim_time_ns, dt_ns, rng, share, bits, I_min=-5.0, I_max=5.0, quantize=True, schedule_mode="random"):
...
def gillespie_sampler_with_energy(J, h, beta_fn, tau_ns, t_end_ns, rng, share, bits, I_min=-5.0, I_max=5.0, apply_delay_ns=None, quantize=True):
    n = J.shape[0]
    m = rng.choice(np.array([-1, 1], dtype=np.int8), size=n)
...
def build_maxcut_ising(adj: np.ndarray, weight_scale: float = 1.0):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if w == 0.0:
                continue
            J[i, j] = J[j, i] = -0.5 * weight_scale * w
...
def run_single_configuration(bits, share, n_pbit, tau, *, sampler, schedule, beta0, beta1, steps, sim_time_ns, dt_ns, repeats, I_min, I_max, problem_spec, rng_seed, density, log_invalid=False, dump_spins=False, quantize=True, apply_delay_ns=None, tick_mode="random"):
    ...
    for r in range(repeats):
        if sampler == "tick":
            m_final, times, E_list = tick_sampler_with_energy(J, h, beta_fn, tau, sim_time_ns, dt_ns, rng, share, bits, I_min=I_min, I_max=I_max, quantize=quantize, schedule_mode=tick_mode)
        elif sampler == "gillespie":
            m_final, times, E_list = gillespie_sampler_with_energy(J, h, beta_fn, tau, sim_time_ns, rng, share, bits, I_min=I_min, I_max=I_max, quantize=quantize, apply_delay_ns=apply_delay_ns)
        else:
...
def simulate_and_plot_list(bits_list, share_list, n_pbit_list, tau_ns_list, *, sampler: str, sim_time_ns=5000.0, dt_ns=1.0, repeats=5, schedule="const", beta0=1.0, beta1=1.0, steps=1, I_min=-5.0, I_max=5.0, problem_spec=None, rng_seed=0, density=0.1, dump_spins=False, quantize=True, apply_delay_ns=None, tick_mode="random", dump_trace=False, trace_dir=None):
    ...
    for bits, share, n_pbit, tau in itertools.product(bits_list, share_list, n_pbit_list, tau_ns_list):
        cnt += 1
        record, energy_runs, time_ref = run_single_configuration(bits, share, n_pbit, tau,
                                                                 ...
                                                                 density=density, log_invalid=True, dump_spins=dump_spins, quantize=quantize, apply_delay_ns=apply_delay_ns, tick_mode=tick_mode)
